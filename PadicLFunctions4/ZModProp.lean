...
TODO (optional)
`eq_of_nhds_eq_nhds` changed to `TopologicalSpace.ext_nhds`
...
lemma proj_fst [Fact p.Prime] {n : ℕ} (x : ZMod d × ℤ_[p]) (cop : d.Coprime (p^n)) :
  ZMod.cast ((ZMod.chineseRemainder cop).symm (x.fst, (toZModPow n) x.snd)) = x.fst := proj_fst' _ _ _
...
by
  have h2 : ZMod.castHom (show m.lcm n ∣ m * n by simp [Nat.lcm_dvd_iff]) (ZMod m × ZMod n) _ = _ := (ZMod.chineseRemainder h).right_inv (a,b)
...
lemma proj_snd [Fact p.Prime] {n : ℕ} (x : ZMod d × ℤ_[p]) (cop : d.Coprime (p^n)) :
  ZMod.cast ((ZMod.chineseRemainder cop).symm (x.fst, (toZModPow n) x.snd)) =
  (toZModPow n) x.snd := proj_snd' _ _ _

lemma inv_fst' {m n : ℕ} (x : ZMod (m * n)) (cop : m.Coprime n) :
  (((ZMod.chineseRemainder cop).toEquiv) x).fst = (ZMod.cast x : ZMod m) :=
by simp [ZMod.chineseRemainder]

lemma inv_fst {n : ℕ} (x : ZMod (d * p^n)) (cop : d.Coprime (p^n)) :
  (((ZMod.chineseRemainder cop).toEquiv) x).fst = (ZMod.cast x : ZMod d) := inv_fst' x _

lemma inv_snd' {m n : ℕ} (x : ZMod (m * n)) (cop : m.Coprime n) :
  (((ZMod.chineseRemainder cop).toEquiv) x).snd = (ZMod.cast x : ZMod n) :=
by simp [ZMod.chineseRemainder]

lemma inv_snd {n : ℕ} (x : ZMod (d * p^n)) (cop : d.Coprime (p^n)) :
  (((ZMod.chineseRemainder cop).toEquiv) x).snd = (ZMod.cast x : ZMod (p^n)) := inv_snd' _ _
...
lemma val_coe_val_le_val' {n m : ℕ} [NeZero m] [NeZero n] (y : ZMod n) :
  (ZMod.cast y : ZMod m).val ≤ y.val := (@ZMod.natCast_val _ (ZMod m) _ _ y) ▸ val_coe_val_le_val y

lemma coe_val_eq_val_of_lt {n m : ℕ} [NeZero n] (h : n < m) (b : ZMod n) :
  (b.val : ZMod m).val = b.val :=
by
  simp only [natCast_val]
  have h2 : b.val = (ZMod.cast b : ZMod m).val := by
    have h2 : b.val < m := by
      transitivity n
      apply ZMod.val_lt b
      assumption
    rw [←ZMod.val_cast_of_lt h2]
    refine congr_arg _ (ZMod.natCast_val _)
  conv_rhs => { rw [h2] }
...
namespace ZMod
lemma natCast_val_to_int {n : ℕ} [NeZero n] (a : ZMod n) : (a.val : ℤ) = (ZMod.cast a : ℤ) :=
by rw [←natCast_val a]

lemma coe_int_add {n : ℕ} [NeZero n] (a b : ZMod n) :
  (ZMod.cast ((a + b) : ZMod n) : ℤ) = (a.cast + (b.cast : ℤ)) % n :=
by
  rw [←natCast_val_to_int, val_add]
...
lemma coe_add_eq_pos' {n : ℕ} {a b : ZMod n} (h : (a.cast + b.cast : ℤ) < n) :
  (((a + b) : ZMod n).cast : ℤ) = (a.cast : ℤ) + (b.cast : ℤ) :=
by
  rw [ZMod.cast_add_eq_ite, if_neg]
  push_neg
  assumption
...
by
  ...
  rw [mul_assoc, ←mul_add d, ←mul_one_add, pow_succ' p m, mul_comm (p^m) _]
  congr 1 -- ring_nf does not work sadly
  rw [Nat.add_sub_cancel' (le_of_lt (fact_iff.1 (Nat.Prime.one_lt' p)))]

lemma nat_cast_ZMod_cast_int {n a : ℕ} (h : a < n) : ((a : ZMod n).cast : ℤ) = (a : ℤ) :=
by
  by_cases h' : NeZero n
  · rw [←ZMod.natCast_val _]
    congr
...
lemma cast_int_one {n : ℕ} [Fact (1 < n)] : ((1 : ZMod n).cast : ℤ) = 1 :=
by
  rw [←ZMod.natCast_val _, ZMod.val_one _]
  simp -- proof got shorter!

lemma cast_eq_of_dvd {m n : ℕ} (h : m ∣ n) (a : ZMod m) : ((a.cast : ZMod n).cast : ZMod m) = a := by  -- shouldnt this lemma be named RingHom..
conv_rhs => { rw [←ZMod.ringHom_map_cast (ZMod.castHom h (ZMod m)) a] }
congr

lemma fract_eq_val {n : ℕ} [NeZero n] (a : ZMod n) :
  Int.fract ((a.cast : ℚ) / n) = (a.val : ℚ) / n :=
...
lemma IsUnit_of_is_coprime_dvd {a b : ℕ} (h : a ∣ b) {x : ℕ} (hx : x.Coprime b) :
  IsUnit (x : ZMod a) := by
  convert_to IsUnit ((ZMod.unitOfCoprime _ hx : ZMod b).cast : ZMod a)
  · rw [←cast_natCast h x]
    congr
...
lemma cast_inv {a m n : ℕ} (ha : a.Coprime n) (h : m ∣ n) :
  (((a : ZMod n)⁻¹ : ZMod n).cast : ZMod m) = ((a : ZMod n).cast : ZMod m)⁻¹ :=
by
  change ((((ZMod.unitOfCoprime a ha)⁻¹ : Units (ZMod n)) : ZMod n).cast : ZMod m) = _
...
by
  ...
  obtain ⟨z, hz⟩ := dvd_sub_comm.mp (modEq_iff_dvd.1 ((eq_iff_modEq_nat _).1 h))
  refine' ⟨z, _⟩
  have h : ∀ z : ℕ, (z : ℚ) = ((z : ℤ) : ℚ) := λ z => by norm_cast
  rw [h a, h b, ← Int.cast_sub, hz, Int.cast_mul, ← h n, mul_comm, mul_div_cancel_of_invertible]

lemma dvd_val_sub_cast_val {m : ℕ} (n : ℕ) [NeZero m] [NeZero n] (a : ZMod m) :
  n ∣ a.val - (a.cast : ZMod n).val :=
by
  have : (a.val : ZMod n) = ((a.cast : ZMod n).val : ZMod n) := by
    rw [natCast_val, natCast_val]
    simp only [cast_id', id_eq]
  exact dvd_iff_mod_eq_zero.2 (sub_mod_eq_zero_of_mod_eq ((eq_iff_modEq_nat _).1 this))
...
TopologicalSpace.ext_nhds ( λ x => by -- eq_of_nhds_eq_nhds
...
lemma coprime_of_IsUnit {m a : ℕ} (ha : IsUnit (a : ZMod m)) : Nat.Coprime a m := by
  have : m ∣ (a - (a : ZMod m).val) := by
    rw [← ZMod.natCast_zmod_eq_zero_iff_dvd, Nat.cast_sub (ZMod.val_le_self _ _), sub_eq_zero]
    cases m
    · rfl
    · rw [ZMod.natCast_val, ZMod.cast_natCast']
...
by rw [←ZMod.cast_natCast h, ZMod.natCast_self, ZMod.cast_zero]
...
(((ZMod.chineseRemainder (Nat.Coprime.pow_right n hd)).invFun (↑(a.fst), ↑(a.snd))).cast : ZMod d) = a.fst :=
proj_fst' _ _ _

lemma proj_snd'' [Fact p.Prime] {n : ℕ} (hd : d.Coprime p) (a : (ZMod d)ˣ × (ZMod (p^n))ˣ) :
(PadicInt.toZModPow n) (((ZMod.chineseRemainder (Nat.Coprime.pow_right n hd)).invFun (↑(a.fst), ↑(a.snd))).cast : ℤ_[p]) = a.snd := by
  rw [← ZMod.intCast_cast, map_intCast, ZMod.intCast_cast]
  simp only [RingEquiv.toEquiv_eq_coe, Equiv.invFun_as_coe, intCast_cast]
...
lemma IsUnit_of_IsUnit_mul {m n : ℕ} (x : ℕ) (hx : IsUnit (x : ZMod (m * n))) :
  ...
  cases' hx with k hk
  refine' ⟨(k.cast : ZMod m), _⟩
  rw [← ZMod.cast_natCast (dvd_mul_right m n), ← ZMod.cast_mul (dvd_mul_right m n), ← hk, ZMod.cast_one (dvd_mul_right m n)]
...
lemma not_IsUnit_of_not_IsUnit_mul' {m n : ℕ} [NeZero (m * n)] (x : ZMod (m * n))
  (hx : ¬ IsUnit x) : ¬ IsUnit (x.cast : ZMod m) ∨ ¬ IsUnit (x.cast : ZMod n) :=
by
  rw [← ZMod.cast_id _ x, ← ZMod.natCast_val] at hx
