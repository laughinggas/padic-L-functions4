...
theorem bernoulli_eval_mul' (m : ℕ) {k : ℕ} (hk : k ≠ 0) (x : ℚ) :
  ...
  { symm
    ...
    { rw [RingHom.map_sub, exp_pow_eq_rescale_exp]
      --congr
      simp only [map_one] } } -- apply (rescale (k : ℚ)).map_one'
